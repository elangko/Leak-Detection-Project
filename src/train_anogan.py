...
from src.models.anogan import GANTrainer, evaluate, optimize_latent
from src.utils import compute_metrics, get_device, get_threshold, save_pickle, set_seed


def parse_args():
    p = argparse.ArgumentParser(description="AnoGAN 학습/평가")
    ...
    p.add_argument("--latent-steps", type=int, default=200, help="테스트 시 latent z 최적화 스텝 수")
    ...
    p.add_argument("--output", type=str, default="./results/anogan_result.pkl")
    return p.parse_args()
...
def run_train_eval(args, device) -> None:
    ...
    trainer = GANTrainer(args, dataloader_train, device)
    D, G, _, _ = trainer.train()

    valid_latent = optimize_latent(args, G, D, dataloader_valid, device, steps=args.latent_steps)
    labels, scores = evaluate(G, D, valid_latent, dataloader_valid, device)
    threshold = get_threshold(scores, labels)

    test_latent = optimize_latent(args, G, D, dataloader_test, device, steps=args.latent_steps)
    labels, scores = evaluate(G, D, test_latent, dataloader_test, device)
