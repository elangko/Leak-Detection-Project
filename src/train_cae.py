...
from src.models.autoencoder import CAETrainer, evaluate
from src.utils import compute_metrics, get_device, get_threshold, save_pickle, set_seed


def parse_args():
    p = argparse.ArgumentParser(description="Autoencoder(CAE) 학습/평가")
    ...
    p.add_argument("--lr-milestones", type=int, nargs="+", default=[500])
    ...
    p.add_argument("--output", type=str, default="./results/cae_result.pkl")
    return p.parse_args()
...
def run_train_eval(args, device) -> None:
    ...
    trainer = CAETrainer(args, dataloader_train, device)
    net = trainer.train()

    labels, scores = evaluate(net, dataloader_valid, device)
    threshold = get_threshold(scores, labels)

    labels, scores = evaluate(net, dataloader_test, device)
